...
class Episode(dict):
    ...
    def get_air_time(self):
        ...
        if firstaired and "-" in firstaired:
            try:
                return long(time.mktime(time.strptime(firstaired, '%Y-%m-%d')))
            except:
                pass
        return -1
...
class Tvdb:
    def __init__(self, api_key, language="en", cache=".", use_thewiz=False):
        config = {}        
        
        self.use_thewiz = use_thewiz
    ...
    def url_sid_full(self, sid, language):
        if self.use_thewiz:
            return "http://apitv.thewiz.info/series/{0}/all/{1}.zip".format(sid, language)
        else:
